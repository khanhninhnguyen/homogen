...
predictiver_rule_ver3 <- function(significance_level,
  ...
  Z_trunc_final_code = construct_logical_table(prob, keep_config, remove_var = NA, list_name_test,
                                               path_save = path_restest)
  List_names_tot = colnames(Z_trunc_final_code)
  List_names_final = List_names_tot[-rm.ind]
  print(List_names_final)
...
predictiver_rule_ver3 <- function(significance_level,
  ...
  colnames(Data_Res_Test)[4:9] <- paste0("t", List_names_tot) 
  colnames(Data_Res_Test_fillNA)[4:9] <- paste0("t", List_names_tot) 
  
  #' check the configuration based on test only 
  t_cols <- colnames(Data_Res_Test)[4:9]
  
  # Perform the operations
  Data_Res_coded <- Data_Res_Test_fillNA %>%
    mutate(across(all_of(t_cols),
                  ~ {
                    # Step 1: Convert t-value to p-value
                    p_value <- 2 * pnorm(-abs(.x))
  ...
  # find the truth based on test 
  Z_truth <- sapply(c(1:nrow(Data_Res_coded)), function(x) {
    config_list_final[which(duplicated2(rbind(Z_trunc_final, 
                                              Data_Res_coded[x,])))[1]]
  })
  ...
  for (b in 1:B){
    ...
    ######
    # Vfold 
    existing_pop_Learn1 <- 0
    existing_pop_Test1 <- 0
    existing_pop_Learn2 <- 0
    existing_pop_Test2 <- 0
    
    
    while ((existing_pop_Learn1!=15) & (existing_pop_Test1 !=15) &
           (existing_pop_Learn2!=15) & (existing_pop_Test2 !=15)){
      trainIndex <- createDataPartition(1:length_data, p = 0.8, list = FALSE)
      GEIndex <- which(Data_Res_Test_fillNA$tGE>=0)
      
      # Loop through each dataset name
      for (name in List_names_final) {
        # Construct the variable names for learning and testing sets
        learn_name1 <- paste("Data", name, "Learn1", sep = "_")
        test_name1 <- paste("Data", name, "Test1", sep = "_")
        learn_name2 <- paste("Data", name, "Learn2", sep = "_")
        test_name2 <- paste("Data", name, "Test2", sep = "_")
        
        learn_ind1 <- intersect(trainIndex, GEIndex)
        learn_ind2 <- intersect(trainIndex, c(1:length_data)[-GEIndex])
        test_ind1 <- intersect(c(1:length_data)[-trainIndex], GEIndex)
        test_ind2 <- intersect(c(1:length_data)[-trainIndex], c(1:length_data)[-GEIndex])
        
        # Slice the data for learning and testing sets
        assign(learn_name1, slice(get(paste("Data", name, sep = "_")), learn_ind1))
        assign(learn_name2, slice(get(paste("Data", name, sep = "_")), learn_ind2))
        assign(test_name1, slice(get(paste("Data", name, sep = "_")), test_ind1))
        assign(test_name2, slice(get(paste("Data", name, sep = "_")), test_ind2))
      ...
      existing_pop_Learn1 <- calculate_unique_pop_sum("Learn1", List_names_final)
      existing_pop_Test1 <- calculate_unique_pop_sum("Test1", List_names_final)
      existing_pop_Learn2 <- calculate_unique_pop_sum("Learn2", List_names_final)
      existing_pop_Test2 <- calculate_unique_pop_sum("Test2", List_names_final)
    }
    
    #####
    # Construction of the pop of t.values for each series Learn and Test
    for (i in 1:length(List_names_final)){
      eval(parse(text=paste0("Pop_",List_names_final[i],"_Learn1=Pop.create(Data_",List_names_final[i],"_Learn1)")))
      eval(parse(text=paste0("Pop_",List_names_final[i],"_Test1=Pop.create(Data_",List_names_final[i],"_Test1)")))
      eval(parse(text=paste0("Pop_",List_names_final[i],"_Learn2=Pop.create(Data_",List_names_final[i],"_Learn2)")))
      eval(parse(text=paste0("Pop_",List_names_final[i],"_Test2=Pop.create(Data_",List_names_final[i],"_Test2)")))
    }
    ...
    Boot <- function(type_dataset, Z_trunc_code, NbSim, List_names_final){
      ...
      for (c in 1:Nbconfig){
        print(c)
        groupGE <- ifelse(c %in% c(1:14, 29:33), 1, 2)
        config_code <- Z_trunc_code[c,]
        config_true = as.numeric(rownames(config_code))
        ...
        if (type_dataset=="Learn"){
          ...
          if (length(true_row) > Nb_config_c){
            ...
            data_c <- purrr::map(List_names_final,~{
              Pop <- c()
              code_names_series <- config_code[which(colnames(Z_trunc_code) %in% .x)]
              eval(parse(text = paste0("Pop=Pop_",.x,"_",type_dataset,groupGE ,"[[",code_names_series,"]]")))
              if(length(Pop)>Nb_config_c){
                res.t = sample(Pop, Nb_config_c, replace = FALSE) # maybe change here 
              }else{
                res.t = sample(Pop, Nb_config_c, replace = TRUE) # maybe change here 
              }
              return(res.t)
            }) %>% 
              bind_cols() %>% 
    ...
    DataLearn <- Boot(type_dataset = "Learn", Z_trunc_code = Z_trunc_final_code, NbSim, List_names_final)
    DataTest <- Boot(type_dataset = "Test", Z_trunc_code = Z_trunc_final_code, NbSim, List_names_final)
